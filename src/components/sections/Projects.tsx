          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className="bg-[#12121a]/85 backdrop-blur-md rounded-[24px] p-10 lg:p-12 border border-white/5 hover:border-violet-500/50 hover:shadow-[0_0_40px_-10px_rgba(124,58,237,0.35)] flex flex-col h-full group"
              >
                <div className="mb-8">
                  <span className="inline-block px-4 py-1.5 bg-violet-500/10 text-violet-400 rounded-full text-xs font-bold mb-5 border border-violet-500/15 tracking-wide uppercase">
                    {project.category}
                  </span>
                  <h3 className="text-2xl lg:text-3xl font-extrabold text-[#f5f5f5] group-hover:text-cyan-400 transition-colors leading-tight">
                    {project.title}
                  </h3>
                </div>
                
                <p className="text-[#8b8b9e] text-sm lg:text-base flex-grow mb-8 leading-relaxed">
                  {project.description}
                </p>
